...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.features == 'M' or self.features == 'MS':
            # features = M: all features, S: only target feature
            # features = MS: all features + target feature
            cols_data = df_raw.columns[1:]
            df_data = df_raw[cols_data]
        elif self.features == 'S':
            df_data = df_raw[[self.target]]
        ...
        if self.timeenc == 0:
            ...
            # timeenc = 1: use time_features function to encode
            data_stamp = time_features(pd.to_datetime(
                df_stamp['date'].values), freq=self.freq)
            data_stamp = data_stamp.transpose(1, 0)
    ...
    def __getitem__(self, index):
        ...
        # r_begin: the start index of the label sequence
